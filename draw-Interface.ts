...
export function drawInterface(
  ...
  chromaticParams?: ChromaticAberrationParams,
  ...
  for (let i = -earthRadius; i <= earthRadius; i += 20) {
    ...
    if (y >= earthY - earthRadius && y <= earthY + earthRadius) {
      const halfWidth = Math.sqrt(
        earthRadius * earthRadius - (y - earthY) * (y - earthY)
      );
  ...
  for (let angle = 0; angle < Math.PI; angle += Math.PI / 6) {
    ...
    ctx.ellipse(
      earthX,
      earthY,
      earthRadius * Math.abs(Math.cos(angle)),
      earthRadius,
      0,
      0,
      Math.PI * 2
  ...
  orbits.forEach((orbit) => {
    ...
    const satelliteAngle =
      ((timeNow % orbit.speed) / orbit.speed) * Math.PI * 2;
    ...
    ctx.lineTo(
      Math.cos(trailAngle) * orbit.radiusX,
      Math.sin(trailAngle) * orbit.radiusY
    );
    ctx.stroke();
  ...
  altOrbits.forEach((orbit) => {
    ...
    ctx.lineTo(
      satelliteX + orbit.satelliteSize,
      satelliteY + orbit.satelliteSize
    );
    ctx.lineTo(
      satelliteX - orbit.satelliteSize,
      satelliteY + orbit.satelliteSize
  ...
  ctx.font = `12px ${fonts.primary}`;
  ctx.fillText(
    `${Math.round(techParams.energyLevel).toString()}`,
    canvas.width - 140,
    120
  );
  ...
  ctx.font = `12px ${fonts.primary}`;
  ctx.fillText(
    `X: ${(Math.round(targetSize * 10) / 100).toFixed(2)}`,
    canvas.width - 140,
    185
  );
  ctx.fillText(
    `Y: ${(Math.round(scanParams.scanResolution * 100) / 10).toFixed(2)}`,
    canvas.width - 140,
    205
  );
  ctx.fillText(
    `Z: ${(Math.round(techParams.insertionDepth * 10) / 10).toFixed(1)}`,
    canvas.width - 140,
    225
  );
...
function generateSystemData(currentTime: number): SystemMonitoringData {
  ...
  const noiseLevels = Array(100)
    ...
    .map(
      (_, i) =>
        Math.sin(currentTime / 1000 + i * 0.2) * 12 +
  ...
  const fuelCells = Array(8)
    .fill(0)
    .map((_, i) =>
      Math.min(100, Math.max(0, 70 + Math.sin(currentTime / 10000 + i) * 30))
    );
...
function updateSystemDataSmooth(
  systemData: SystemMonitoringData,
  currentTime: number
): void {
  ...
  systemData.signalStrength = Math.min(
    100,
    Math.max(20, systemData.signalStrength)
  );
...
function drawASCIIChart(
  ...
  data.forEach((value, index) => {
    ...
    ctx.fillText(
      `CELL ${index + 1}: ${barText} ${Math.round(value)}%`,
      x,
      y + index * 12
    );
  });
